...
function sendViewedMessage(messageChannel, id, videoPath) {
    ...
    let videoId = `viewed-${id}`
    messageChannel.publish(videoId, "", Buffer.from(jsonMsg)); // Publish message to the "viewed" exchange.
}
...
function setupHandlers(app, messageChannel) {
    app.get("/video", (req, res) => { // Route for streaming video-1.
        ...
        if (videoId == 1) {
            ...
            videoPath = "./videos/SampleVideo_1280x720_1mb copy.mp4";
        }
        ...
        fs.stat(videoPath, (err, stats) => {
            ...
            sendViewedMessage(messageChannel, videoId, videoPath); // Send message to "history" microservice that this video has been "viewed-1".
        });
    });
}
